...
section('15. Integración LAB — panel, otros motores intactos, sin producción');
{
  ...
  const panels = readFileSync(path.join(root, 'js/arpa-ia/arpa-ia-panels.js'), 'utf8');
  const registry = readFileSync(path.join(root, 'js/arpa-ia/arpa-ia-registry.js'), 'utf8');
  const cacheCfg = readFileSync(path.join(root, 'js/arpa-ia/arpa-ia-cache.js'), 'utf8');
  assert(htmlApp.indexOf('id="arpa-ia-slot-historial"') >= 0, '15 panel en Historial');
  assert(/ARPA IA COMERCIAL/.test(panels), '15 título del panel');
  assert(htmlApp.indexOf('arpa-ia-bootstrap.js') >= 0 && registry.indexOf('comercial-ui.js') >= 0, '15 carga comercial-ui.js');
  assert(panels.indexOf('id="formato-ia-tecnica"') >= 0, '15 IA Técnica OT sigue en el formato');
